...
import { Plus, Trash2, Play, Pause, RotateCcw, ArrowUp, ArrowDown, Timer, Clock, AlertTriangle, X, ExternalLink } from 'lucide-react';
...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'tuesday' | 'thursday'>('all');
  ...
  const fetchData = async () => {
    try {
      ...
      if (sessionsRes.ok) {
        ...
        const tueIds = rawSessions.filter(s => s.training_day === 'tuesday').map(s => s.id);
        const thuIds = rawSessions.filter(s => s.training_day === 'thursday').map(s => s.id);
        setDayOrders({
          tuesday: tueIds,
          thursday: thuIds,
        });
  ...
  const moveItem = (day: 'tuesday' | 'thursday', index: number, direction: 'up' | 'down') => {
  ...
  const addToSession = async (drillId: string, day: 'tuesday' | 'thursday') => {
    try {
      const res = await fetch('/api/sessions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ drill_id: drillId, training_day: day }),
      });
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try {
      const res = await fetch(`/api/drills/delete?id=${id}`, { method: 'DELETE' });
      if (res.ok) fetchData();
    } catch (err) {
      console.error(err);
    }
  ...
  const isDrillInSession = (drillId: string, day: 'tuesday' | 'thursday') => {
    return sessions.some((s) => s.drill_id === drillId && s.training_day === day);
  };

  const getOrderedSessionItems = (day: 'tuesday' | 'thursday') => {
  ...
  const calculateTotalTime = (day: 'tuesday' | 'thursday') => {
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-20">
      <header className="bg-emerald-800 text-white p-4 shadow-md sticky top-0 z-40 border-b border-emerald-900">
        <div className="max-w-3xl mx-auto flex justify-between items-center gap-3">
          <div className="flex items-center gap-3">
            <div className="bg-emerald-900 text-emerald-100 font-black text-xs px-2 py-1.5 rounded-lg border border-emerald-700 tracking-wider">
              WCF.C.
            </div>
            <div>
              ...
              <p className="text-[11px] text-emerald-200 font-medium">Football Drills</p>
            </div>
          </div>
          <button
            onClick={() => setIsModalOpen(true)}
            className="bg-white text-emerald-900 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold flex items-center gap-1 shadow hover:bg-emerald-50 transition shrink-0"
          >
      ...
      <div className="max-w-3xl mx-auto px-4 pt-4">
        <div className="flex bg-slate-200 p-1 rounded-xl gap-1">
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${
              activeTab === 'tuesday' ? 'bg-white text-emerald-800 shadow' : 'text-slate-600'
            }`}
          >
            Tuesday ({sessions.filter((s) => s.training_day === 'tuesday').length})
          </button>
          <button
            onClick={() => setActiveTab('thursday')}
            className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${
              activeTab === 'thursday' ? 'bg-white text-emerald-800 shadow' : 'text-slate-600'
            }`}
          >
            Thursday ({sessions.filter((s) => s.training_day === 'thursday').length})
          </button>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-4">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {drills.map((drill) => {
              const inTuesday = isDrillInSession(drill.id, 'tuesday');
              const inThursday = isDrillInSession(drill.id, 'thursday');
                ...
                <div key={drill.id} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-2 mb-2">
                      ...
                      <button onClick={() => handleDelete(drill.id, drill.title)} className="text-slate-400 hover:text-red-600 p-1">
                  ...
                  <div className="space-y-2 mt-2">
                    <div className="flex gap-2">
                      <button
                        onClick={() => addToSession(drill.id, 'tuesday')}
                      ...
                      >
                        {inTuesday ? '✓ Tuesday' : '+ Tue Session'}
                      </button>
                      <button
                        onClick={() => addToSession(drill.id, 'thursday')}
                        className={`flex-1 text-[11px] font-bold py-1.5 rounded-lg border transition ${
                          inThursday
                            ? 'bg-emerald-600 text-white border-emerald-600'
                            : 'bg-slate-100 hover:bg-emerald-50 text-slate-700 border-slate-200'
                      ...
                      >
                        {inThursday ? '✓ Thursday' : '+ Thu Session'}
                      </button>
                    </div>
          ...
          <div className="space-y-4">
            ...
            {(() => {
              ...
              const diff = totalMins - 60;

              return (
                <div className="bg-slate-900 text-white p-4 rounded-xl shadow-md space-y-2">
                  <div className="flex flex-wrap justify-between items-center gap-3">
                    <div>
                      <h2 className="text-sm font-bold capitalize">{activeTab} Session Plan</h2>
                  ...
                  <div className="text-[11px] font-semibold pt-1 border-t border-slate-800 flex justify-between">
                    {diff > 0 ? (
                      ...
                      <span className="text-emerald-400">💡 {Math.abs(diff)} mins remaining in your 60m session budget.</span>
                    ) : (
                      <span className="text-emerald-300">✓ Perfect! Exactly 60 minutes planned.</span>
                    )}
                  </div>
                </div>
              );
            ...
            {getOrderedSessionItems(activeTab).length === 0 ? (
              <div className="text-center py-10 bg-white rounded-xl border border-slate-200 p-4">
                <p className="text-slate-500 font-medium text-sm">No drills added to {activeTab.toUpperCase()} yet.</p>
                <p className="text-slate-400 text-xs mt-1">Switch to "All Drills" tab and tap "+ {activeTab === 'tuesday' ? 'Tue' : 'Thu'} Session".</p>
              </div>
            ) : (
      ...
      <AddDrillModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onDrillAdded={fetchData} />
    </main>
  );
}
